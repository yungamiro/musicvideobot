...
import { deleteRoom, getPlayback, setPlayback } from "./store.js";

const app = express();
app.use(cors({ origin: config.activityOrigin }));
app.use(express.json({ limit: "64kb" }));

function isBotAuthorized(authorization: string | undefined): boolean {
  return authorization === `Bearer ${config.botApiKey}`;
}
...
app.post("/api/token", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Discord token exchange failed", payload);
      res.status(502).json({ error: "Discord OAuth exchange failed" });
      return;
    }
    res.json(payload);
  } catch (error) {
    console.error(error);
    res.status(502).json({ error: "Discord OAuth is unreachable" });
  }
});
...
app.put("/api/rooms/:roomId/playback", (req, res) => {
  if (!isBotAuthorized(req.header("authorization"))) {
  ...
  if (!body.success) {
    res.status(400).json({ error: "Invalid playback payload", details: z.treeifyError(body.error) });
    return;
  }
